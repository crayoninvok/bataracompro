  ...
  const striveValues = [
    {
      ...
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 text-[#1FBFB8]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          {striveValues.map((value, index) => (
            <div key={index} className="bg-orange-50 rounded-lg p-6 hover:shadow-md transition-all">
              <div className="flex items-start mb-4">
                <div className="w-16 h-16 bg-[#1FBFB8]/10 rounded-full flex items-center justify-center mr-4 flex-shrink-0">
                  {value.icon}
                </div>
                <div className="text-left">
                  <div className="flex items-center mb-2">
                    <span className="text-3xl font-bold text-[#E85C23] mr-2">{value.letter}</span>
                    <h3 className="text-xl font-bold text-[#5B5B5F]">{value.title}</h3>
                  </div>
