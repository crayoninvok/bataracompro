...
function GoalCard({ title, description, icon, color }: GoalCardProps): React.ReactNode {
  return (
    <div className="bg-black/40 p-6 rounded-lg border border-gray-800 hover:border-gray-700 transition-all duration-300 shadow-lg group">
      <div className="flex items-start">
        <div className={`bg-${color}/10 p-4 rounded-lg mr-4 group-hover:bg-${color}/20 transition-colors`}>
          {icon}
        </div>
        <div>
          <h3 className={`font-bold text-xl mb-2 text-white group-hover:text-${color} transition-colors`}>
            {title}
          </h3>
  ...
  const goals = [
    {
      ...
      description: "Continuously improve our mining and transportation processes to achieve industry-leading efficiency and reliability.",
      icon: <Target className="w-6 h-6 text-[#E85C23]" />,
      color: "[#E85C23]"
    },
    {
      title: "Sustainable Growth",
      description: "Expand our operational capacity while maintaining our commitment to environmental responsibility and community development.",
      icon: <TrendingUp className="w-6 h-6 text-[#1FBFB8]" />,
      color: "[#1FBFB8]"
    },
    {
      title: "Community Partnership",
      description: "Build strong relationships with local communities through employment opportunities, education initiatives, and collaborative projects.",
      icon: <Users className="w-6 h-6 text-[#E85C23]" />,
      color: "[#E85C23]"
    },
    {
      title: "Safety Leadership",
      description: "Maintain the highest standards of workplace safety and health, with zero tolerance for preventable incidents.",
      icon: <Shield className="w-6 h-6 text-[#1FBFB8]" />,
      color: "[#1FBFB8]"
    }
  ];
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16 reveal-project opacity-0 translate-y-6 transition-all duration-700">
          <h2 className="text-3xl font-bold mb-4 relative inline-block">
            <span className="relative z-10 text-white">Our Strategic Goals</span>
            <span className="absolute -bottom-2 left-0 w-full h-3 bg-[#E85C23]/20 z-0"></span>
          </h2>
          <p className="text-lg text-gray-300 max-w-3xl mx-auto">
            Guiding principles that drive our commitment to excellence in mining and transportation operations
          </p>
        </div>
